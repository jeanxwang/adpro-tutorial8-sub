...
impl MessageHandler<UserCreatedEventMessage> for UserCreatedHandler {
    fn handle(&self, message: Box<UserCreatedEventMessage>) -> Result<(), HandleError> {
        ...
        println!("Aliyah Faza Qinthara - 2206024726 [129500004y]. Message received: {:?}", message);
        Ok(())
    }

    fn get_handler_action(&self) -> String {
        // return a string that describes the action of the handler
        "Handle UserCreatedEventMessage".to_string()
    }
}
